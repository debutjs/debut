 ...
 */
type RangeExtractionResult<
  NodeType extends lowlight.HastNode = lowlight.HastNode
> =
  | number
  | {
      text: string;
...
function extractAstAtRangeFromNode<NodeType extends lowlight.HastNode>(
  offset: number,
  length: number,
  astSpecificType: NodeType,
): RangeExtractionResult<NodeType> {
  ...
  switch (ast.type) {
    ...
    case 'root':
      ...
      for (const innerAst of ast.children) {
        const extractedAst: RangeExtractionResult =
          textInChildren == null
            ? // We have not started getting useful text, so we move our start point
              extractAstAtRangeFromNode(
                offset - charactersSkipped,
                length,
                innerAst as lowlight.HastNode,
              )
            : // We are progressively filling up with text, so we reduce our length
              extractAstAtRangeFromNode(
                0,
                length - textInChildren.length,
                innerAst as lowlight.HastNode,
              );
 ...
 */
export function getRangeFromParsedAsts(
  offset: number,
  length: number,
  asts: lowlight.HastNode[],
) {
  const ranges = extractAstAtRangeFromNode(offset, length, {
    type: 'root',
    children: asts,
  });
...
export function lowlightAstToReactComponent(
  ast: lowlight.HastNode,
): React.ReactNode {
  switch (ast.type) {
    case 'text':
      return ast.value;
    case 'element':
      return React.createElement(
        ast.tagName,
        ast.properties,
        ...(ast.children as lowlight.HastNode[]).map(
